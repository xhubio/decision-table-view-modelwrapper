...
import {
  TESTCASE_NAME_PREFIX,
  FIELD_SECTION_NAMES,
  MULTI_ROW_SECTION_NAMES,
...
export default class ViewWrapper {
  ...
  getRowFor(number) {
    ...
    throw new Error(
      `The index '${number}' is out of range. Only '${this.rowOrder
        .length}' rows available.`
    )
  }
  ...
  getRowTypeFor(rowNumber) {
    ...
    throw new Error(
      `The index '${rowNumber}' is out of range. Only '${this.rowOrder
        .length}' rows available.`
    )
  }
  ...
  getSectionTypeFor(rowNumber) {
    ...
    throw new Error(
      `The index '${rowNumber}' is out of range. Only '${this.rows
        .length}' rows available.`
    )
  }
  ...
  _createRow(rowId) {
    ...
    if (rowData.rowType === DATA_ROW) {
      ...
      if (prentRowType === FIELD_SUB_SECTION) {
        if (parentRowData.section.equivalenceClasses[rowId]) {
          row.c2 = parentRowData.section.equivalenceClasses[rowId]
        }
        if (parentRowData.section.tdgs[rowId]) {
          row.c3 = parentRowData.section.tdgs[rowId]
        }
        if (parentRowData.section.comments[rowId]) {
          row.c4 = parentRowData.section.comments[rowId]
        }
      } else if (prentRowType === MULTI_ROW_SECTION) {
        // MultiRowSection
        if (parentRowData.section.keys[rowId]) {
          row.c2 = parentRowData.section.keys[rowId]
        }
        if (parentRowData.section.others[rowId]) {
          row.c3 = parentRowData.section.others[rowId]
        }
        if (parentRowData.section.comments[rowId]) {
          row.c4 = parentRowData.section.comments[rowId]
        }
      ...
      // MultiRowSection
      if (rowData.section.total) {
        row.c2 = rowData.section.total + ' Total testcases'
      }
      if (rowData.section.done) {
        row.c3 = rowData.section.done + ' Done testcases'
      }
      if (rowData.section.percent) {
        row.c4 = rowData.section.percent + '%'
      }
    }
    return row
  }
  ...
  _buildSectionRows(section, parentId) {
    ...
    this.logger.debug({
      message: `Work on section ${section.name}`,
      sectionType: section.sectionType,
      function: '_buildSectionRows',
      sheet: this.table.name,
    })
